...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--strict-mapping",
        action="store_true",
        help="严格模式：检测到未映射Excel列时直接终止，避免自动透传未知字段",
    )
...
def get_unmapped_excel_columns(rows: list[dict[str, str]], field_mapping: dict[str, str]) -> list[str]:
    """返回未配置映射的Excel列（忽略控制列，大小写不敏感）。"""
    if not rows:
        return []

    mapped_columns = {column.casefold() for column in field_mapping}
    unmapped: list[str] = []
    for column_name in rows[0].keys():
        normalized_name = column_name.strip()
        if not normalized_name:
            continue
        if is_control_excel_column(normalized_name):
            continue
        if normalized_name.casefold() in mapped_columns:
            continue
        unmapped.append(normalized_name)
    return unmapped
...
def build_runtime_config(args: argparse.Namespace, file_config: dict[str, Any]) -> dict[str, Any]:
    ...
    auto_map_unmapped_fields = bool(file_config.get("auto_map_unmapped_fields", True))
    strict_mapping = bool(file_config.get("strict_mapping", False)) or bool(args.strict_mapping)
    ...
    return {
        ...
        "auto_map_unmapped_fields": auto_map_unmapped_fields,
...
def is_control_excel_column(column_name: str) -> bool:
    return column_name.strip().casefold() in CONTROL_EXCEL_COLUMNS
...
def build_payload(
    ...
    auto_map_unmapped_fields: bool = True,
) -> dict[str, Any]:
    ...
    if auto_map_unmapped_fields:
        mapped_columns = {column.casefold() for column in field_mapping}
        for excel_column, value in row.items():
            if value == "":
                continue
            if excel_column.casefold() in mapped_columns:
                continue
            if is_control_excel_column(excel_column):
                continue

            api_field = excel_column.strip()
            if not api_field or api_field in payload:
                continue
            payload[api_field] = format_field_value(api_field, value)
    return payload
...
def extract_invalid_fields_from_error(error_message: str) -> set[str]:
    """解析错误信息中无效字段名：如 [stepping] is NOT a valid field for ..."""
    invalid_fields: set[str] = set()
    if not error_message:
        return invalid_fields

    pattern = re.compile(r"\[([^\]]+)\]\s+is\s+NOT\s+a\s+valid\s+field", re.IGNORECASE)
    for matched in pattern.findall(error_message):
        field_name = matched.strip()
        if field_name:
...
def create_ticket(
    ...
    raw_payload = build_payload(row, field_mapping, static_fields, auto_map_unmapped_fields)
...
def main() -> None:
    ...
    unmapped_columns = get_unmapped_excel_columns(rows, runtime_config["field_mapping"])
    if unmapped_columns:
        preview = ", ".join(unmapped_columns[:10])
        suffix = " ..." if len(unmapped_columns) > 10 else ""
        if runtime_config["strict_mapping"]:
            raise ValueError(
                "检测到未映射Excel列，严格模式已阻止提交。"
                "请在 field_mapping 中补充映射或移除这些列后重试。"
                f" 未映射列: {preview}{suffix}"
            )
        if runtime_config["auto_map_unmapped_fields"]:
            log.warning(f"⚠️ 检测到未映射Excel列，将按同名字段自动透传: {preview}{suffix}")
        else:
            log.warning(f"⚠️ 检测到未映射Excel列，且已禁用自动透传，这些列将被忽略: {preview}{suffix}")
    ...
    for index, row in enumerate(rows, start=args.start_row):
        ...
        if not args.dry_run and row_ticket_id:
            ...
            desired_fields = build_payload(
                ...
                auto_map_unmapped_fields=runtime_config["auto_map_unmapped_fields"],
            )
                ...
                if fetch_esservice_error:
                    ...
                    if updates:
                        ...
                        if not updated:
                            ...
                            if update_esservice_error:
                                invalid_fields = extract_invalid_fields_from_error(update_esservice_error)
                                if invalid_fields:
                                    retry_updates = {
                                        key: value
                                    ...
                                    if retry_updates:
                                        log.info("  ↻ 重新提交更新（已移除无效字段）")
                                        retry_ok, retry_response, retry_error = update_ticket_fields(
                                            api_url=runtime_config["api_url"],
                                            headers=headers,
                                            timeout=args.timeout,
                                            insecure=args.insecure,
                                            ...
                                            subject=subject,
                                            ticket_id=row_ticket_id,
                                            update_fields=retry_updates,
                                        )
                                        if retry_ok:
                                            retry_esservice_error = extract_esservice_error(retry_response)
                                            if not retry_esservice_error:
                                                log.info(f"  ✅ 更新成功（已跳过无效字段）  ticket_id={row_ticket_id}")
                                                result = {
                                                    "success": True,
                                                    "ticket_id": row_ticket_id,
                                                    "error": "",
                                                    "payload": retry_updates,
                                                    "response": retry_response,
                                                    "action": "updated",
                                                    "skipped_invalid_fields": sorted(invalid_fields),
                                                }
                                            else:
                                                result = {
                                                    "success": False,
                                                    "ticket_id": row_ticket_id,
                                                    "error": f"更新失败: {retry_esservice_error}",
                                                    "payload": retry_updates,
                                                    "response": retry_response,
                                                    "action": "update_failed",
                                                }
                                            ...
                                            result = {
                                                "success": False,
                                                "ticket_id": row_ticket_id,
                                                "error": f"更新失败: {retry_error}",
                                                "payload": retry_updates,
                                                "action": "update_failed",
                                            }
                                    else:
                                        log.info("  ℹ️ 仅检测到无效字段变更，已跳过本次更新")
                                        result = {
                                            "success": True,
                                            "ticket_id": row_ticket_id,
                                            "error": "",
